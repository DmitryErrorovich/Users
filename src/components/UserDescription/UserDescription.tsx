...
const UserDesc = ({
  ...
  touched
  // }: any) => {
}: IProps & FormikProps<IFormValues>): any => {
  ...
  return (
    <Card elevation={6} className={"User_card"}>
      <CardMedia className={"User_media"} image={get(user, "picture.large")} />
      <CardContent>
      ...
      <CardActions className={"User_actions"}>
        <Button onClick={expandEdit} size="small" color="primary">
          Edit
        </Button>
        <Button
      ...
      >
        <CardContent>
          <form onSubmit={handleSubmit}>
            ...
            <Button
              disabled={updatingUser !== ILoading.SUCCEEDED || !dirty}
              color="primary"
              variant="contained"
              fullWidth
              type="submit"
...
const formikEnhance = withFormik<IProps, IFormValues>({
  ...
  handleSubmit: async (
    { first, last, city, country }: IFormValues,
    formikBag
  ) => {
    await formikBag.props.editUsers({
      ...formikBag.props.user,
      name: { ...formikBag.props.user.name, first, last },
      location: { ...formikBag.props.user.location, city, country }
    });
  }
});
